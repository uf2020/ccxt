...
Besides making basic market and limit orders, some exchanges offer margin trading (leverage), various derivatives (like future contracts and options) and also have [dark pools](https://en.wikipedia.org/wiki/Dark_pool), [OTC](https://en.wikipedia.org/wiki/Over-the-counter_(finance)) (over-the-counter trading), merchant APIs and much more.
...
- `market.secret / market['secret'] / $market->secret`: Your secret API key string literal, [see below](https://github.com/kroitor/ccxt/wiki/Manual#api-keys-setup). Most markets require this for trading as well.
...
In most cases you are required to load the list of products and trading symbols for a particular market prior to accessing other market API methods. If you forget to load products you will have exceptions of all sorts. In order to load products call the `loadProducts ()` / `load_products ()` method on a market instance. It returns an associative array of products indexed by trading symbol.
